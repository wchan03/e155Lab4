...
void enableDelayTimer(TIMER_TypeDef * TIMERx) { //used for delay
  ...
  // prescale register
  TIMERx->TIMx_PSC = 39; //TODO: set prescaler to 80. 100kHz clock?
...
void delay_millis(TIMER_TypeDef * TIMERx, uint32_t ms){
  ...
  TIMERx->TIMx_SR &= ~(1 << 0); //TODO: does this work?

  //set counter to 0 
  TIMERx->TIMx_CNT = 0;
  
  while((TIMERx->TIMx_SR & 1) == 0); //wait for flag
